...
from utils.os_util import ctrl_c
...
class UiDebugClipBoardWatch(QObject):
    ...
    @staticmethod
    def install_on_q_object(obj: QObject):
        """ Installs an event filter on the given QObject to copy a meaningful value to the clipboard when clicked """
        obj.eventFilter = UiDebugClipBoardWatch()
        QApplication.instance().installEventFilter(obj.eventFilter)
        QApplication.instance().aboutToQuit.connect(obj.cleanup_event_filter)
